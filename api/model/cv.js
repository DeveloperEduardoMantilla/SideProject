...
export class cvModel{
    ...
    static async getCvUser(id){
        return new Promise((resolve,reject)=>{
            database.query(
                "SELECT cv.*, JSON_OBJECT( 'id', en.id, 'nombre', en.nombre) AS enfoque, JSON_OBJECT( 'usuario', u.usuario, 'fechaRegistro', u.fechaRegistro, 'rol', u.rol, 'genero', u.genero, 'estado', u.estado, 'telefono', u.telefono, 'correo', u.correo, 'ciudad', u.ciudad) AS info_usuario FROM cv INNER JOIN usuario u ON u.id = cv.idUsuario INNER JOIN enfoque en ON cv.idEnfoque = en.id WHERE u.rol = 'usuario' AND u.id = ?",
                [id],
                (err,data)=>{
                    if(err) return resolve({status:400, message: err.message})
                    return resolve({status:200, message:data})
    ...
    static async filterTecnoCv(tecn){
        return new Promise((resolve,reject)=>{
            database.query(
                "SELECT cv.*, JSON_OBJECT( 'id', en.id, 'nombre', en.nombre) AS enfoque, JSON_OBJECT( 'usuario', u.usuario, 'fechaRegistro', u.fechaRegistro, 'rol', u.rol, 'genero', u.genero, 'estado', u.estado, 'telefono', u.telefono, 'correo', u.correo, 'ciudad', u.ciudad) AS info_usuario FROM cv INNER JOIN usuario u ON u.id = cv.idUsuario INNER JOIN enfoque en ON cv.idEnfoque = en.id WHERE u.rol = 'usuario' AND u.estado = 1 AND cv.estado = 1 AND JSON_CONTAINS(cv.skills, ?)",
                [JSON.stringify(tecn)],
                (err,data)=>{
                    if(err) return resolve({status:400, message: err.message})
                    return resolve({status:200, message:data})
    ...
    static async filterRutaCv(rut){
        return new Promise((resolve,reject)=>{
            database.query(
                "SELECT cv.*, JSON_OBJECT( 'id', en.id, 'nombre', en.nombre) AS enfoque, JSON_OBJECT( 'usuario', u.usuario, 'fechaRegistro', u.fechaRegistro, 'rol', u.rol, 'genero', u.genero, 'estado', u.estado, 'telefono', u.telefono, 'correo', u.correo, 'ciudad', u.ciudad) AS info_usuario FROM cv INNER JOIN usuario u ON u.id = cv.idUsuario INNER JOIN enfoque en ON cv.idEnfoque = en.id WHERE u.rol = 'usuario' AND u.estado = 1 AND cv.estado = 1 AND cv.idEnfoque = ?",
                [rut],
                (err,data)=>{
                    if(err) return resolve({status:400, message: err.message})
                    return resolve({status:200, message:data})
    ...
    static async filterIngCv(ing){
        return new Promise((resolve,reject)=>{
            database.query(
                "SELECT cv.*, JSON_OBJECT( 'id', en.id, 'nombre', en.nombre) AS enfoque, JSON_OBJECT( 'usuario', u.usuario, 'fechaRegistro', u.fechaRegistro, 'rol', u.rol, 'genero', u.genero, 'estado', u.estado, 'telefono', u.telefono, 'correo', u.correo, 'ciudad', u.ciudad) AS info_usuario FROM cv INNER JOIN usuario u ON u.id = cv.idUsuario INNER JOIN enfoque en ON cv.idEnfoque = en.id WHERE u.rol = 'usuario' AND u.estado = 1 AND cv.estado = 1 AND cv.nivelIdioma = ?",
                [ing],
                (err,data)=>{
                    if(err) return resolve({status:400, message: err.message})
                    return resolve({status:200, message:data})
